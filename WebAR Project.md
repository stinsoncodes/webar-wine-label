...
Task 2: Implement the Chroma-Key Shader (chromakey.js)
* Write a custom A-Frame component/shader (e.g., AFRAME.registerShader('chromakey', {...})).
* The shader must take the video texture and key out the green background (RGB roughly 0, 255, 0), returning alpha 0.0 for green pixels and alpha 1.0 for the character.
* Apply this shader to an <a-plane> nested inside the MindAR target entity so the video maps onto a flat plane over the bottle.
